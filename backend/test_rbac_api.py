...
def test_rbac_import_checks():
    ...
    try:
        ...
        print("✅ RBAC schema imports successful")
...
def test_rbac_service_functionality():
    ...
    try:
        ...
        print(f"✅ Created/retrieved admin user: {admin_user.username}")
        print(f"✅ Created/retrieved regular user: {regular_user.username}")
        ...
        print(f"✅ Regular user can create users: {user_can_create_users}")
        ...
        print(f"❌ RBAC service test failed: {e}")
        import traceback
        traceback.print_exc()
        return False
    finally:
...
def test_api_endpoints_signatures():
    ...
    try:
        ...
        print(f"✅ Registered RBAC routes: {len(rbac_routes)}")
...
def test_schema_validation():
    ...
    try:
        ...
        print(f"✅ RoleCreate schema works: {role_data.name}")
        ...
        print(f"✅ PermissionCreate schema works: {perm_data.name}")
        ...
        print(f"✅ UserRoleAssignment schema works: user_id={assignment_data.user_id}, role_id={assignment_data.role_id}")
        ...
        print(f"✅ PermissionCheckRequest schema works: {check_data.permission}")
...
def test_database_method_signatures():
    ...
    try:
        ...
        try:
            ...
            print(f"✅ get_highest_hierarchy_level works: {hierarchy_level}")
        except Exception as e:
            print(f"❌ get_highest_hierarchy_level failed: {e}")
            return False
        
        try:
            user_roles = user.get_roles(db)
            print(f"✅ get_roles works: {len(user_roles)} roles")
        except Exception as e:
            print(f"❌ get_roles failed: {e}")
            return False
        
        try:
            has_perm = user.has_permission("documents:read", db)
            print(f"✅ has_permission works: {has_perm}")
        except Exception as e:
            print(f"❌ has_permission failed: {e}")
            return False
        
        return True
        
    except Exception as e:
        print(f"❌ Database method signatures test failed: {e}")
        import traceback
        traceback.print_exc()
        return False
    finally:
